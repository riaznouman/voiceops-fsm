...
export default function SkillsPage() {
  ...
  async function handleSave(e: FormEvent) {
    ...
    try {
      ...
      const r = editTarget
        ? await fetch(`/api/skills/${editTarget.id}`, {
            method: "PUT",
  ...
  return (
    <>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="mb-1 text-2xl font-bold text-gray-900">Skills</h1>
          <p className="text-sm text-gray-500">Manage technician skills</p>
        </div>
        <button
          type="button"
          onClick={openCreate}
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={3} className="px-3 py-10 text-center text-sm text-gray-500">No skills yet.</td>
              </tr>
            ) : (
              skills.map((s) => (
                <tr key={s.id} className="hover:bg-gray-50">
                  <td className={`${bodyCellClass} font-medium text-gray-900`}>{s.name}</td>
                  <td className={bodyCellClass}>{s.description ?? "—"}</td>
                  <td className={`${bodyCellClass} text-right`}>
                    <div className="flex justify-end gap-1.5">
                      <button type="button" onClick={() => openEdit(s)} className="rounded border border-gray-300 p-1.5 text-gray-500 hover:bg-gray-100">
                        <Pencil size={13} />
                      </button>
                      <button type="button" onClick={() => handleDelete(s.id)} className="rounded border border-gray-300 p-1.5 text-red-400 hover:bg-red-50">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </td>
      ...
      <Modal title={editTarget ? "Edit skill" : "Add skill"} open={modalOpen} onClose={() => setModalOpen(false)}>
